...
def _employee_subquery(user_id: int):
    """Sous-requête : employee_id de l'utilisateur connecté."""
...
async def get_my_events(
    ...
    result = await db.execute(
        ...
        .where(CalendarEvent.employee_id == emp_id)
...
async def get_my_calendar_history(
    ...
    emp_id = _employee_subquery(current_user["user_id"])
    result = await db.execute(
        select(CalendarEventLog)
        .where(CalendarEventLog.employee_id == emp_id)
        .order_by(desc(CalendarEventLog.created_at))
        .offset(offset)
        .limit(limit)
    )
...
async def get_my_leave_history(
    ...
    result = await db.execute(
        select(LeaveLog)
        .where(LeaveLog.employee_id == emp_id)
        .order_by(desc(LeaveLog.created_at))
        .offset(offset)
        .limit(limit)
    )

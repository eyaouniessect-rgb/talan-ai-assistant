...
import { CheckCircle, FileText, Eye, ArrowRight, ShieldAlert, ShieldCheck,
         ...
         ChevronDown, ChevronRight, Pencil, Trash2, X, Save } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      ...
      {Object.entries(grouped).map(([epicIdx, epicStories]) => {
        ...
        // Review data : pris depuis la première story (identique pour tout l'epic)
        const review   = epicStories[0]?._review ?? null;

        return (
          <div key={epicIdx} className="rounded-xl border border-slate-200 overflow-hidden">
            {/* Header epic */}
            <div className="flex items-center gap-2 px-4 py-2.5 bg-navy/5 border-b border-slate-200">
            ...
            {/* Bandeau review (scope creep / qualité) */}
...
export default function PhaseResult({ phaseId, aiOutput }) {
  ...
  if (phaseId === "stories") {
    return <StoriesSection aiOutput={aiOutput} />;
  }
